...
public class MemberController {
	...
	public void searchById(String userId) {
		...
		Member m =new MemberDao().searchById(userId); //m (조회된 회원정보)
		
		//조회된 회원정보가 담긴 객체 m이 null이면 조회되지 않은 상태
		if(m==null) { //비어있으면 = 조회되지 않으면
	...
	public void searchByName(String userName) {
		...
		if(list.isEmpty()) { //(list가 비어있니? true) = 비어있는경우
			new MemberView().displayNodata(userName+"에 대한 조회 결과가 없습니다.");
		}
		else {//조회가 된 경우->조회결과있음(list에 Member객체가 담겨있음) 
			new MemberView().displayList(list);
		}
...
//		new MemberDao().updateMember(userId,userPw,email,phone,address);
		
		int result=new MemberDao().updateMember(m);
		
		//view 성공 실패시 보여줄 화면 결정
		if(result>0) {
			//성공했으니 성공메세지를 보낸다
